...
@WebServlet("/teacher_mana_indexServlet")
public class Teacher_Mana_IndexServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //查询班级数目
        int count = adminServices.loadTeacherNum();
        ...
        teacherList = adminServices.loadAllTeacherByPage(current_page,page_size);
        if(teacherList !=null){
            request.setAttribute("teacherList",teacherList);
            request.setAttribute("current_page",current_page);
            request.setAttribute("page_num",page_num);
            request.getRequestDispatcher("/admin/teacher_management/teacher_mana_index.jsp").forward(request,response);
        }else{
